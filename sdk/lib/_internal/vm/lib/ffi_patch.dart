...
import "dart:_internal"
    show patch, has63BitSmis, UnmodifiableListMixin, FixedLengthListMixin;
import 'dart:async';
...
extension Int8Array on Array<Int8> {
  ...
  Int8List get elements {
    ...
    return Int8List.view(typedData.buffer, start, length);
  }
}
...
extension Int16Array on Array<Int16> {
  ...
  Int16List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Int16List.view(typedData.buffer, start, length);
  }
}
...
extension Int32Array on Array<Int32> {
  ...
  Int32List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Int32List.view(typedData.buffer, start, length);
  }
}
...
extension Int64Array on Array<Int64> {
  ...
  Int64List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Int64List.view(typedData.buffer, start, length);
  }
}
...
extension Uint8Array on Array<Uint8> {
  ...
  Uint8List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Uint8List.view(typedData.buffer, start, length);
  }
}
...
extension Uint16Array on Array<Uint16> {
  ...
  Uint16List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Uint16List.view(typedData.buffer, start, length);
  }
}
...
extension Uint32Array on Array<Uint32> {
  ...
  Uint32List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Uint32List.view(typedData.buffer, start, length);
  }
}
...
extension Uint64Array on Array<Uint64> {
  ...
  Uint64List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Uint64List.view(typedData.buffer, start, length);
  }
}
...
extension FloatArray on Array<Float> {
  ...
  Float32List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Float32List.view(typedData.buffer, start, length);
  }
}
...
extension DoubleArray on Array<Double> {
  ...
  Float64List get elements {
    ...
    final typedData = _typedDataBase as TypedData;
    final start = typedData.offsetInBytes + _offsetInBytes;
    return Float64List.view(typedData.buffer, start, length);
  }
}
...
class _BoolArrayList with ListMixin<bool>, FixedLengthListMixin<bool> {
  ...
  @override
  bool operator [](int index) => _array[index];
...
extension PointerArray<T extends NativeType> on Array<Pointer<T>> {
  ...
  @patch
  List<Pointer<T>> get elements => _PointerArrayList(this);
}

class _PointerArrayList<T extends NativeType>
    with ListMixin<Pointer<T>>, FixedLengthListMixin<Pointer<T>> {
  _PointerArrayList(this._array);

  final Array<Pointer<T>> _array;

  @override
  int get length => _array._size;

  @override
  Pointer<T> operator [](int index) => _array[index];
...
/// Wrapper used by ArrayArray.elements after its been rewritten by the FFI transformer.
class _ArrayArrayList<T extends NativeType>
    with ListMixin<Array<T>>, FixedLengthListMixin<Array<T>> {
  _ArrayArrayList(this._array, int _singleElementSize)
    : _elementSize = _singleElementSize * _array._nestedDimensionsFlattened;

  final Array<Array<T>> _array;
  ...
  @override
  Array<T> operator [](int index) {
    _array._checkIndex(index);
    return Array<T>._(
      _array._typedDataBase,
      _array._offsetInBytes + (_elementSize * index),
      _array._nestedDimensionsFirst,
      _array._variableLength,
      _array._nestedDimensionsRest,
    );
  }

  @override
  operator []=(int index, Array<T> value) {
    _array._checkIndex(index);
    _memCopy(
      _array._typedDataBase,
      _array._offsetInBytes + (_elementSize * index),
      value._typedDataBase,
      value._offsetInBytes,
      _elementSize,
    );
  }
}
...
/// Wrapper used by StructArray.elements and UnionArray.elements after its been
/// rewritten by the FFI transformer.
class _ArrayList<T extends NativeType>
    with ListMixin<T>, UnmodifiableListMixin<T> {
  _ArrayList(this._array, this._elementSize, this._constructor);

  final Array<T> _array;
  final int _elementSize;
  final T Function(Object typedDataBase, int offsetInBytes) _constructor;
  ...
  T operator [](int index) {
    ...
    return _constructor(
      _array._typedDataBase,
      _array._offsetInBytes + index * _elementSize,
    );
  }
}
...
/// Wrapper used by StructArray.elements and UnionArray.elements after its been
/// rewritten by the FFI transformer.
class _AbiSpecificIntegerArrayList<T extends AbiSpecificInteger>
    with ListMixin<int>, FixedLengthListMixin<int> {
  _AbiSpecificIntegerArrayList(
    this._array,
    this._loadAbiSpecificInt,
    this._storeAbiSpecificInt,
  );

  final Array<T> _array;
  final int Function(Array<T> array, int index) _loadAbiSpecificInt;
  final int Function(Array<T> array, int index, int value) _storeAbiSpecificInt;
  ...
  int operator [](int index) {
    _array._checkIndex(index);
    return _loadAbiSpecificInt(_array, index);
  }

  @override
  operator []=(int index, int value) {
    _array._checkIndex(index);
    _storeAbiSpecificInt(_array, index, value);
  }
}
